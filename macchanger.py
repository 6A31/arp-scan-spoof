...
def change_mac_windows(new_mac):
    ...
    new_mac = new_mac.split(":")
    a = "".join(new_mac).upper()
    ...
    while True:
        ...
        if int(update_option) >= 0 and int(update_option) < len(mac_to_change_to):
            print(f"{Fore.BLUE}Your Mac Address will be changed to: {Fore.GREEN}{mac_to_change_to[int(update_option)]} {Style.RESET_ALL}")
            break
        else:
